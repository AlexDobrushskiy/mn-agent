...
class MnAPITestCase(TestCase):
    ...
    def test_create_mn(self):
        ...
        r = self.client.put('/api/masternode/', data={"ip": "127.0.0.1", "address": "asdasd", "balance": 222},
    ...
    def test_update_mn(self):
        ...
        self.assertEqual(Masternode.objects.count(), 1)
        r = self.client.put('/api/masternode/', data={"ip": mn.ip, "address": "NEW ADDR", "balance": 999},
                            content_type='application/json')
        self.assertEqual(r.status_code, 200)
        self.assertEqual(Masternode.objects.count(), 1)
